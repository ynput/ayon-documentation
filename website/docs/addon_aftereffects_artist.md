...
To install the extension, download, install [Anastasyi's Extension Manager](https://install.anastasiy.com/). Open Anastasyi's Extension Manager and select AfterEffects in menu. Then go to `{path to AE addon}hosts/aftereffects/api/extension.zxp`.
...
![Extension](assets/aftereffects_extension.png) <!-- same menu as in PS -->
...
Name of publishable instance (eg. product name) could be configured with a template in `ayon+settings://core/tools/creator/product_name_profiles`.
