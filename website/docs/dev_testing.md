...
For the frontend E2E tests we're using <a href="https://playwright.dev/" target="_blank" rel="noreferrer">Playwright</a> which proves to be useful and easy to use toolset for developers and even less technical users. In order to run tests locally you will need to install playwright and follow the installation instructions (defaults are safe choices):
```
...
As mentioned previously We chose <a href="https://playwright.dev/" target="_blank" rel="noreferrer">Playwright</a> for its versatility and ease of use.
